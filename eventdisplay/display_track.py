...
# open the LSTNtuple file
f = r.TFile("LSTNtuple.root")
t = f.Get("tree")

sim = Visualization.DisplaySimTrack(tree=t, ievt_to_display=0, isim_to_display=3)

Visualization.create_3d_figures(
        objs_to_draw = sim.get_simtrk_goScatter3Ds(),
        output_name = "htmls/display_track_3d_ievt_{}_isim_{}.html".format(sim.ievt, sim.isim)
        )
...
# # Some interesting list of detids
# all_detids = list(geom.getDetIds(lambda x: Module(x[0]).isLower()==1))
# barrel_flat_detids = list(geom.getDetIds(lambda x: Module(x[0]).isBarrelFlat()==1))

# # plotly graphical object for all detector modules
...
# # plotly graphical object for barrel flat modules
